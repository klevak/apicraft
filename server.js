...
var api_model = __dirname + '/api_model.json';

/* Our API Blueprint */
var api_blueprint = __dirname + '/api_blueprint.txt';
...
fs.readFile(api_blueprint, 'utf8', function (err, data) {
...
function createApp(obj){
	...
	console.log("routes are:" + app.routes.length)
	...
	*/
	...
	app.get('/blueprint', function(req, res){
	 protagonist.parse(api_blueprint, function(error, result) {
		if (error) {
			res.send(error);
			return;
		}
...
fs.readFile(api_model, 'utf8', function (err, data) {
	...
	APICraft.newengland.get('/api', function(req, res) {
          res.render('api', { proxyUrl: config.proxyUrl, page: apipage });
        });
